...
#include <iostream>
	...
	void Scene::Draw(int shaderIndx,int cameraIndx,int buffer,bool toClear,bool debugMode)
	{
		...
		glm::mat4 Normal = MakeTrans(); //translation according to scene
	...
	void Scene::MouseProccessing(int button)
	{
		if(pickedShape == -1)
		{
			...
			{				
				MoveCamera(0, xTranslate, xrel / 50.0f);
				MoveCamera(0, yTranslate, -yrel / 50.0f);
				//MyTranslate(glm::vec3(-xrel/50.0f,0,0),0); //positive x-offset moves camera left, i.e moves scene to the right
				//MyTranslate(glm::vec3(0,yrel/50.0f,0),0); //positive y-offset moves camera up, i.e moves scene downwards
				WhenTranslate();
			}
			else //LEFT BUTTON
			{
				MyRotate(-xrel/2.0f,glm::vec3(0,1,0),0); //offset is positive so rotate COUNTER-CLOCKWISE , hence the minus
				MyRotate(-yrel/2.0f,glm::vec3(1,0,0),0);  //offset is positive so rotate COUNTER-CLOCKWISE , hence the minus
				WhenRotate();
			}
		}
	}
